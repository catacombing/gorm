...
use calloop::futures::ExecutorDestroyed;
use calloop::timer::{TimeoutAction, Timer};
use calloop::{EventLoop, LoopHandle, RegistrationToken, channel};
...
async fn main() {
    // Setup logging.
    let directives = env::var("RUST_LOG").unwrap_or("warn,gorm=info".into());
...
impl State {
    fn new(
        ...
        let config_options = ConfigOptions::new("gorm").notify(true);
        ...
        let config =
            config_manager.get::<&str, _>(&[]).inspect_err(|err| error!("Config error: {err}"));
        let config = Rc::new(config.ok().flatten().unwrap_or_default());
...
enum Error {
    ...
    #[error("{0}")]
    EventLoopAsync(#[from] ExecutorDestroyed),
    #[error("{0}")]
